public class CustomStringMethods {
  ...
  */
    public String pigLatin(String orig) {
        orig = orig.substring(1) + orig.substring(0, 1) + "ay";
        return orig;
    }
  ...
  /**Client provides myString and removeIdx and method returns a new String with the character
    ...
    public String removeCharacter(String myString, int removeIdx) {
        if (removeIdx<myString.length()) {
            myString = myString.substring(0, removeIdx) + myString.substring(removeIdx+1);
            return myString;
        } else {
            return myString;
        }
  ...
  */
    public String insertAt(String orig, String insertText, String searchStr) {
        if (orig.indexOf(searchStr)>=0) {
            orig = orig.substring(0, orig.indexOf(searchStr)) + insertText + orig.substring(orig.indexOf(searchStr));
            return orig;
        } else {
            orig += insertText;
            return orig;
  ...
  */
    public String endUp(String myString, int numToCap){
        if (numToCap<myString.length()) {
            myString = myString.substring(0, myString.length()-numToCap) + myString.substring(myString.length()-numToCap).toUpperCase();
            return myString;
        } else {
